--select * from Albums
--select * from Track

--select al.title as Albums, mt.name as mediatype, g.name as genre
--from Track t join Albums al on al.AlbumsId= t.AlbumsId join MediaType mt on mt.MediaTypeId=t.MediaTypeId
--join Genre g on g.GenreId


--select at.Name as Artistsname, count(t.trackid) as TrackCount from
--track t join Albums al on t.AlbumsId=al.AlbumsId
--join
--Artists at on at.ArtistsId=at.ArtistsId
--group by at.Name
--order by TrackCount desc

CREATE TABLE RecordLabel( 

LabelID int not null, 

Name varchar(50) not null 

); 

 

 

 

INSERT INTO RecordLabel VALUES(1,'Blackened'); 

INSERT INTO RecordLabel VALUES(3,'Universal'); 

INSERT INTO RecordLabel VALUES(4,'MCA'); 

INSERT INTO RecordLabel VALUES(5,'Elektra'); 

INSERT INTO RecordLabel VALUES(2,'Warner Bros'); 

 

 

 

ALTER TABLE recordLabel 

ADD CONSTRAINT PK_reclabel 

PRIMARY KEY (labelid); 

 

 

 

 

CREATE TABLE Artists ( 

LabelId int not null, 

ArtistsId int not null, 

Name varchar(50) not null); 

INSERT INTO Artists VALUES(1,1,'Metallica'); 

INSERT INTO Artists VALUES(1,2,'Megadeth'); 

INSERT INTO Artists VALUES(1,3,'Anthrax'); 

INSERT INTO Artists VALUES(2,4,'Eric Clapton'); 

INSERT INTO Artists VALUES(2,5,'ZZ Top'); 

INSERT INTO Artists VALUES(2,6,'Van Halen'); 

INSERT INTO Artists VALUES(3,7,'Lynyrd Skynyrd'); 

INSERT INTO Artists VALUES(3,8,'ACDC'); 

 

 

 

ALTER TABLE Artists 

ADD CONSTRAINT PK_Artists 

PRIMARY KEY (Artistsid); 

 

 

 

ALTER TABLE Artists 

ADD CONSTRAINT fk_label 

FOREIGN KEY (labelid) 

REFERENCES recordLabel (labelid); 

 

CREATE TABLE Albums ( 

ArtistsId int not null, 

AlbumsId int not null, 

Name varchar(50) not null, 

Year int not null 

); 

INSERT INTO Albums VALUES(1,1,'...And Justice For All',1988); 

INSERT INTO Albums VALUES(1,2,'Black Albums',1991); 

INSERT INTO Albums VALUES(1,3,'Master of Puppets',1986); 

INSERT INTO Albums VALUES(2,4,'Endgame',2009); 

INSERT INTO Albums VALUES(2,5,'Peace Sells',1986); 

INSERT INTO Albums VALUES(3,6,'The Greater of 2 Evils',2004); 

INSERT INTO Albums VALUES(4,7,'Reptile',2001); 

INSERT INTO Albums VALUES(4,8,'Riding with the King',2000); 

INSERT INTO Albums VALUES(5,9,'Greatest Hits',1992); 

INSERT INTO Albums VALUES(6,10,'Greatest Hits',2004); 

INSERT INTO Albums VALUES(7,11,'All-Time Greatest Hits',1975); 

INSERT INTO Albums VALUES(8,12,'Greatest Hits',2003); 

 

 

 

ALTER TABLE Albums 

ADD CONSTRAINT PK_Albums 

PRIMARY KEY (Albumsid); 

 

ALTER TABLE Albums 

ADD CONSTRAINT fk_Artists 

FOREIGN KEY (Artistsid) 

REFERENCES Artists (Artistsid); 

 

 

 

CREATE TABLE Song ( 

AlbumsId int not null, 

SongId int not null, 

Name varchar(50) not null, 

Duration real not null 

); 

INSERT INTO Song VALUES(1,1,'One',7.25); 

INSERT INTO Song VALUES(1,2,'Blackened',6.42); 

INSERT INTO Song VALUES(2,3,'Enter Sandman',5.3); 

INSERT INTO Song VALUES(2,4,'Sad But True',5.29); 

INSERT INTO Song VALUES(3,5,'Master of Puppets',8.35); 

INSERT INTO Song VALUES(3,6,'Battery',5.13); 

INSERT INTO Song VALUES(4,7,'Dialectic Chaos',2.26); 

INSERT INTO Song VALUES(4,8,'Endgame',5.57); 

INSERT INTO Song VALUES(5,9,'Peace Sells',4.09); 

INSERT INTO Song VALUES(5,10,'The Conjuring',5.09); 

INSERT INTO Song VALUES(6,11,'Madhouse',4.26); 

INSERT INTO Song VALUES(6,12,'I am the Law',6.03); 

INSERT INTO Song VALUES(7,13,'Reptile',3.36); 

INSERT INTO Song VALUES(7,14,'Modern Girl',4.49); 

INSERT INTO Song VALUES(8,15,'Riding with the King',4.23); 

INSERT INTO Song VALUES(8,16,'Key to the Highway',3.39); 

INSERT INTO Song VALUES(9,17,'Sharp Dressed Man',4.15); 

INSERT INTO Song VALUES(9,18,'Legs',4.32); 

INSERT INTO Song VALUES(10,19,'Eruption',1.43); 

INSERT INTO Song VALUES(10,20,'Hot For Teacher',4.43); 

INSERT INTO Song VALUES(11,21,'Sweet Home Alabama',4.45); 

INSERT INTO Song VALUES(11,22,'Free Bird',14.23); 

INSERT INTO Song VALUES(12,23,'Thunderstruck',4.52); 

INSERT INTO Song VALUES(12,24,'T.N.T',3.35); 

 

 

 

ALTER TABLE song 

ADD CONSTRAINT PK_song 

PRIMARY KEY (songid); 

 

ALTER TABLE song 

ADD CONSTRAINT fk_Albums 

FOREIGN KEY (Albumsid) 

REFERENCES Albums (Albumsid); 

Select * from Albums
Select * from Song
Select * from Artists
Select * from RecordLabel